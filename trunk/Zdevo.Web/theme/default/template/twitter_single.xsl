	...
	<xsl:template match="zdevo">
		<html>
			<head>
				...
				<title><xsl:value-of select="//data/headtitle" /><xsl:text> | </xsl:text><xsl:value-of select="//data/columnnow" /><xsl:text> | </xsl:text><xsl:value-of select="//data/site/title"/></title>
				<xsl:element name="link">
					<xsl:attribute name="rel">stylesheet</xsl:attribute>
					<xsl:attribute name="rev">stylesheet</xsl:attribute>
					<xsl:attribute name="href"><xsl:value-of select="//data/site/config/variable[@name='SITE_HOST']" />theme/<xsl:value-of select="//data/site/config/variable[@name='SITE_TEMPLATE']" />/code/Css.ashx?color=orange</xsl:attribute>
			...
			<body class="twitter single twitter-single">
				<div id="divAll">
					<div id="divPage">
					<div id="divMiddle">
...
<xsl:apply-templates select="data/twitter" />
...
<!-- 文章输出区 -->
	<xsl:element name="div">
		...
		<h4 class="post-date"><xsl:value-of select="postdatetime" />,<xsl:value-of select="user/name" />用<xsl:value-of select="source" />发布:</h4>
		<div class="post-body"><xsl:value-of select="content" disable-output-escaping="yes" />
		<xsl:if test="link!=''">
